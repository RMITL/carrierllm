/**
 * Comprehensive logging utility for debugging Clerk billing issues
 */
...
class Logger {
  ...
  private addLog(level: LogLevel, message: string, context?: string, data?: any): void {
    ...
    if (level === 'error' && this.shouldReportError(data)) {
  ...
  private getCurrentSessionId(): string | undefined {
    try {
      // Try to get session ID from Clerk
  ...
  private shouldReportError(data?: any): boolean {
    // Only report billing-related errors
    if (data?.context === 'billing' || data?.component === 'PricingTable') {
      return true;
    }
    return false;
  }
  ...
  private async reportError(logEntry: LogEntry): Promise<void> {
    try {
      // Send to your API endpoint for error tracking
      ...
      // Don't log errors from error reporting
...
if (typeof window !== 'undefined') {
  (window as any).carrierLogger = logger;
}
