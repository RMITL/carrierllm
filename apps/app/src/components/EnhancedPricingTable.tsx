...
export const EnhancedPricingTable: React.FC<EnhancedPricingTableProps> = ({
  onError,
  onLoading,
}) => {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      if (!isClerkReady) {
        logger.billingWarn('Clerk billing timeout - switching to fallback mode');
    ...
    const checkClerkAvailability = () => {
      try {
        if (typeof window !== 'undefined' && (window as any).Clerk) {
          logger.billingDebug('Clerk instance found');
          ...
          logger.billingWarn('Clerk instance not found');
        }
      } catch (error) {
        logger.billingError('Error checking Clerk availability', { error });
      }
    };
  ...
  }, [isClerkReady, onError]);

  // Monitor for billing events
  useEffect(() => {
    const handleBillingEvent = (event: any) => {
      logger.billingInfo('Billing event detected', { event });
    };

    const handleBillingError = (event: any) => {
      logger.billingError('Billing error event', { event });
  ...
  const handlePricingTableError = (error: Error) => {
    logger.billingError('PricingTable error', { error: error.message, stack: error.stack });
  ...
  const handleFallbackBilling = async (plan: string) => {
    ...
    try {
      ...
      logger.billingInfo('Redirecting to Clerk checkout', { checkoutUrl, params: params.toString() });
      window.location.href = `${checkoutUrl}?${params}`;
    } catch (error) {
      logger.billingError('Fallback billing failed', { error });
